...
module.exports = class MulterSharpResizer {
  ...
  renameKeys(keysMap, obj) {
    return Object.keys(obj).reduce((acc, key) => {
      ...
      return {
        ...
        orginalFileUrl: `${this.fileUrl}/${this.tmpOriginalname}`,
        field: this.tmpField,
   ...
   * @param  {number} i
   */
  promiseAllResize(file, i, prop = "", filenameParam = file.filename) {
    Promise.all(
      this.sizes.map((size) => {
      // console.log(filenameParam);
        this.imageExt = file.mimetype.split("/")[1];
        this.imageFilename = `${size.path}_${filenameParam.split(/\.([^.]+)$/)[0]}.${this.imageExt}`;
